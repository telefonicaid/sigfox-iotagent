...
function generatePayload(queryParams, device, callback) {
    function decode(data, code, active_attributes, callback) {
        sigfoxParser.createParser(code)(data, active_attributes, callback);
    }

    function createPayload(data, active_attributes, callback) {
        var attributes = [];
    ...
    function createPayloadMetadata(data, active_attributes, callback) {
        var attributes = [];
        var metadatas = [];
        for (var i in queryParams) {
            if (queryParams.hasOwnProperty(i) && i !== 'data' && i !== 'id') {
                var not_found = true;
                for(var p in active_attributes){
                  if(active_attributes[p].name == i){
                    attributes.push({
                        name: i,
                        type: 'String',
                        value: queryParams[i]
        ...
        for(var a in attributes){
          attributes[a]['metadatas'] = metadatas
        }
