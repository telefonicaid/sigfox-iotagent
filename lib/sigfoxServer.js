...
function start(config, callback) {
    ...
    logger.info(context, 'Starting Sigfox server listening on port [%s]', config.sigfox.port);
    logger.debug(context, 'Using config:\n\n%s\n', JSON.stringify(config, null, 4));

    iotServer.app.set('port', config.sigfox.port);
...
function stop(callback) {
    logger.info(context, 'Stopping Sigfox server');
